...
## Latest GMDD Visual And Initial Layout Refinement

- Functional commit: `e2c44307641dec9cf2a45aa56339a17a48443d96`
  (`Improve GMDD visual layout`).
- Start state for this phase:
  - the active checkout was initially on `WiP20261_TINKERCELL`, so a local
    `WiP20261_KERNEL_GUI` branch was created from `origin/WiP20261` to preserve
    the KERNEL_GUI branch policy while starting from the integrated base requested
    by the user;
  - no `origin/WiP20261_KERNEL_GUI` branch was present in this clone at the time
    of this phase.
- Diagnosis:
  - GMDD graphics are represented by
    `GraphicalModelDataDefinition`, with `GraphicalModelComponent` inheriting from
    it for component graphics;
  - GMDD editability for GUI/property-editor purposes is tracked by
    `GraphicalModelDataDefinition::_editableInPropertyEditor`, updated by
    `GraphicalModelBuilder::synchronizeGraphicalDataDefinitionsLayer()`;
  - GMDD color, alpha, border, text, and selection colors flow through
    `TraitsGUI<GModelDataDefinition>` into
    `GraphicalModelDataDefinition::renderContext()` and then into
    `GraphicalModelItemRenderStrategy`;
  - the active GMDD positioning path is the queued scene synchronization through
    `ModelGraphicsScene::requestGraphicalDataDefinitionsSync()` and
    `GraphicalModelBuilder::synchronizeGraphicalDataDefinitionsLayer()`;
  - the older `ModelGraphicsScene::createDiagrams()` path still exists but is not
    the current synchronization entrypoint used by the GUI rebuild/update flow.
- Implementation:
  - moved the non-editable GMDD fill alpha values into
    `TraitsGUI<GModelDataDefinition>` as `nonEditableOpacity` and
    `selectedNonEditableOpacity`;
  - kept text, border, selection handles, and hit shape unchanged so
    non-editable GMDDs become visually softer without losing legibility or
    selectability;
  - extended `GraphicalDataDefinitionLayout::arcPosition()` with an optional
    `radialLayer` parameter;
  - kept editable GMDDs above their owning component and shared/non-editable GMDDs
    below it in `GraphicalModelBuilder`;
  - kept recursive children on the same side as their GMDD parent: children of
    editable GMDDs use the upper arc, while children of non-editable GMDDs use the
    lower arc;
  - added occupied-rectangle tracking in the automatic layout pass, seeded with
    component bounds and already-existing GMDD bounds;
  - newly created GMDDs now try the normal arc position first, then move outward
    through controlled radial layers when that candidate intersects an occupied
    rectangle;
  - existing/persisted GMDD positions are not forcibly rearranged by this phase.
- Tests:
  - added two `GuiDataDefinitionLayout` unit tests proving radial layers keep
    upper children above and lower children below their anchors.
- Validation performed:
  - `cmake --preset tests-kernel-unit` passed;
  - `cmake --build --preset tests-kernel-unit-run` passed, including the updated
    `genesys_test_gui_render_strategy` with 9 GUI render/layout tests;
  - `cmake --preset gui-app` passed;
  - `cmake --build --preset gui-app` passed and linked
    `genesys_qt_gui_application`;
  - `git diff --check` passed;
  - the generated method inventory file was briefly regenerated by validation and
    restored because this phase did not change public kernel API.
- Current state:
  - GMDD non-editable fill is visibly more transparent than editable GMDD fill,
    while text, border, and selection affordances remain fully readable;
  - automatic initial GMDD placement uses upper/lower arcs plus outward radial
    collision fallback for newly created GMDDs;
  - no push was performed.
- Suggested next phase, only after explicit user confirmation: run or build a
