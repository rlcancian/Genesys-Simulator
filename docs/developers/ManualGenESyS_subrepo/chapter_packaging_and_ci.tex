...
The recorded baseline commit is \texttt{d0924e97b4bb00a87b4588832c6e7d11b222ee86}
(from pull request \#524, \texttt{packaging: consolidate GenESyS into one
Debian package}), which supersedes the temporary five-package split delivered
by pull request \#522.
...
\noindent\textbf{Job 1 -- \texttt{Build Debian package}.} It:
\begin{itemize}
...
\item collects the build log, \texttt{.buildinfo}/\texttt{.changes}, and the
  produced \texttt{.deb} file;
\item verifies that exactly one binary package was produced and that it is
  named \texttt{genesys-simulator};
\item validates
  \texttt{packaging/linux/io.github.rlcancian.genesys.metainfo.xml} with
  \texttt{appstreamcli validate --no-net};
\item runs \texttt{lintian --fail-on error} over the single \texttt{.deb}
  file, so any Lintian \texttt{error}-severity finding fails the job while
  warnings are recorded but do not block it;
...
The Debian source package is \texttt{genesys-simulator}, and it produces
exactly one binary package, also named \texttt{genesys-simulator}, declared as
the single \texttt{Package:} stanza in \texttt{debian/control}. This is a
deliberate distinction: \emph{application boundary is not the same thing as
Debian package boundary}. GenESyS still ships several independent
executables -- the Qt GUI, the command-line shell, the HTTP worker, the
legacy \texttt{genesys-web} compatibility command, the stable launcher and
dispatcher, and the public \texttt{genesys-mcp} dispatcher entry point -- but
all of them are installed, versioned, upgraded, and removed as one
distribution unit. There is no per-application Debian package to install,
depend on, or remove separately.

The single \texttt{genesys-simulator} package installs:
\begin{itemize}
\item the stable launcher, the non-interactive dispatcher, and
  \texttt{/etc/genesys/update.conf};
\item the public \texttt{genesys-shell} wrapper and the Debian-provided
  system fallback shell;
\item the public \texttt{genesys-worker} wrapper, the legacy
  \texttt{genesys-web} command alias, and the system fallback worker;
\item the Qt desktop GUI wrapper, the system fallback GUI, and the desktop
  integration files;
\item the public \texttt{genesys-mcp} dispatcher entry point.
\end{itemize}
\texttt{apt install genesys-simulator} alone yields the complete supported
base install; there is no separate package to add for GUI, Shell, or Worker
support.

\texttt{debian/control} declares versioned \texttt{Breaks}/\texttt{Replaces}
relationships against the historical development package names
(\texttt{genesys-common}, \texttt{genesys-shell}, \texttt{genesys-worker},
\texttt{genesys-gui}, and \texttt{genesys-web}). Those relationships exist only
so that a machine with an earlier five-package development installation can
upgrade to the unified package without file-ownership conflicts; none of those
names are produced as binary packages anymore.
...
% Purpose:
%   Show the relationship between the Debian source package, the single
%   genesys-simulator binary package, and the generated
%   diagnostics/package/lifecycle-evidence artifact outputs across both
%   workflow jobs.
% Required visual content:
%   - source package
%   - one genesys-simulator binary package
...
%   - ../packaging/linux/io.github.rlcancian.genesys.metainfo.xml
% Purpose:
%   Show the installed filesystem layout inside the single genesys-simulator
%   binary package: the public wrapper commands, the libexec
%   launcher/dispatcher/system-fallback tree, and the desktop/metainfo/icon
%   files.
% Required visual content:
...
Figure~\ref{fig:packaging-package-layout} should make it obvious that a single
\texttt{genesys-simulator} package owns the stable launcher/dispatcher
mechanism, the administrative policy, and every public wrapper and system
fallback binary. The figure should read as one package fanning out into
several installed executables and files, not as several packages each owning
a slice of the tree.
...
The \texttt{genesys-simulator} package does not install a system fallback for
...
other \texttt{cmake+ninja} package. \texttt{debian/genesys-simulator.install}
then only materializes that single staged tree into the single binary
package; it never copies an executable directly from the build tree, and no
Debian-specific install logic exists outside \texttt{debian/rules} and
\texttt{debian/genesys-simulator.install}/\texttt{.manpages}.
...
single \texttt{genesys-simulator} package produced by Job 1:
\begin{enumerate}
