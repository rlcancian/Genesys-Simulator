...
\begin{figure}[htbp]
    ...
    %   - supported Debian package path (genesys-gui plus companion packages)
...
The packaged desktop application is named \texttt{genesys-gui}. Installing it
pulls in four companion binary packages through ordinary \texttt{apt}
dependency resolution: \texttt{genesys-common} (the stable launcher and
dispatcher mechanism and the administrative update policy),
\texttt{genesys-shell}, and \texttt{genesys-worker}. Together they form the
supported base application set, so \texttt{apt install genesys-gui} is
sufficient to obtain a working GUI, shell, and worker installation.
...
It is a wrapper that starts the stable launcher mechanism installed by
\texttt{genesys-common}, which then starts the actual GUI: either a validated
...
sudo apt install ./genesys-gui_*.deb
\end{lstlisting}
...
live under \path{/usr/libexec/genesys/} and are installed by
\texttt{genesys-common}.
...
\item the Debian-provided \emph{system fallback} runtime, installed under
  \path{/usr/libexec/genesys/system/bin/} by the \texttt{genesys-gui},
  \texttt{genesys-shell}, and \texttt{genesys-worker} packages.
\end{itemize}
...
\item \path{/etc/genesys/update.conf} -- the system-wide administrative
  policy installed by \texttt{genesys-common} as a Debian conffile. It takes
  precedence over the per-user configuration.
...
sudo apt update
sudo apt install --only-upgrade genesys-gui
\end{lstlisting}
...
sudo apt purge genesys-gui
\end{lstlisting}

The historical \texttt{genesys-web} package name is preserved as a transitional
package that depends on \texttt{genesys-worker} and installs no files of its
own, so upgrading an older \texttt{genesys-web} installation keeps working and
the \path{genesys-web} command remains available as a compatibility alias for
\path{genesys-worker}. A system that still has \texttt{genesys-web} explicitly
installed may need to list it alongside the other package names when removing
GenESyS completely; the safe habit is to inspect the package list
(\texttt{dpkg -l 'genesys-*'}) after the first uninstall if the goal is to
