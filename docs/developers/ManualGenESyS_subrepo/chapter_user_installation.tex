...
\begin{figure}[htbp]
    ...
    %   - supported Debian package path
    %   - optional local artifact path
    %   - installed launcher name
    %   - first GUI startup
...
The packaged desktop application is named \texttt{genesys-gui}. The Debian
metadata installs the executable to \path{/usr/bin/genesys-gui} and also ships
the desktop integration files under the standard XDG locations:
\begin{itemize}
...
From a user point of view, the important installed locations are the launcher
binary and the desktop integration files. For package-based installs, the key
paths are:
\begin{itemize}
\item \path{/usr/bin/genesys-gui};
\item \path{/usr/share/applications/io.github.rlcancian.genesys.desktop};
\item \path{/usr/share/metainfo/io.github.rlcancian.genesys.metainfo.xml};
\item \path{/usr/share/icons/hicolor/scalable/apps/io.github.rlcancian.genesys.svg}.
\end{itemize}
...
Because the Debian package also declares \texttt{genesys-web} as a runtime
dependency, a system that installed both packages explicitly may need both names
in the removal command. The safe habit is to inspect the package list after the
first uninstall if the goal is to clean the machine completely.
