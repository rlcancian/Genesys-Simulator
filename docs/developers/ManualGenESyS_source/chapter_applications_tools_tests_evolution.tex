...
In practice, this separation allows the GUI, terminal application, and other future layers to exist as distinct projections of the same set of core services.
...
\subsection{The Terminal Application as an Alternative Form of Use}
\label{subsec:cap10_aplicacao_terminal_forma_alternativa}

The current build tree of the project shows that the terminal application is handled in a particularly structured way. The \texttt{CMakeLists.txt} of \texttt{source/applications/terminal} explains the use of an application called \texttt{GenesysShell}, as well as the possibility of selecting other terminal application classes and even specific example files in \texttt{source/applications/terminal/examples} to compose the terminal executable

This is extremely important for the developer. It shows that the terminal is not just an old auxiliary tool or a marginal way of use. It is an effective application of GenESyS and, more than that, a valuable environment for:
\begin{itemize}
...
In the first half of the book, the examples in \texttt{C++} of the terminal application were deliberately left out, because the user manual should not start with code. Now, however, they begin to occupy a central place. This is because these examples show one of the most important capabilities of GenESyS as software: its use as a simulation library.

The terminal application's \texttt{CMakeLists.txt} explicitly shows the existence of build-selectable examples, including \textit{smart} examples and more elaborate teaching examples, which confirms the role of these files as study material and architectural demonstration
...
\begin{itemize}
 ...
 \item the parser;
 \item at least one concrete application, such as the GUI or the terminal.
\end{itemize}
...
Even though the build is not the guiding axis of the book, it continues to offer the developer an important set of signals about the project organization. The terminal application build file, for example, shows that the system was designed to allow configurable selection of the behavior of the terminal executable, either by specific classes or by concrete examples in \texttt{source/applications/terminal/examples}
...
\begin{table}[htbp]
 ...
 \hline
 Applications & Concrete forms of use, such as GUI and terminal application \\
 \hline
...
This chapter has closed the main structure of the developer's manual by showing that \textit{GenESyS} should not only be read for its central classes, but also for its ecosystem of applications, tools, examples and tests. The GUI now appears as an application on top of the core. The terminal application and its examples in \texttt{C++} reveal the use of the simulator as a library. Tools expand the development environment. The test tree provides the basis for safe evolution. And the general organization of the project shows that GenESyS should be thought of as a software platform, and not just as a ready-made simulator.
...
\item What is the architectural value of the examples in \texttt{C++} of the terminal application?
