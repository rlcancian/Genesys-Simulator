...
Once the compilation is complete, the next objective is to start the graphical application. The generated executable corresponds to the \texttt{genesys\_gui\_application} target, and its runtime name is \texttt{genesys-gui}. From the preset build directory, it is available at \texttt{source/applications/gui/genesys/genesys-gui}.

\begin{lstlisting}[language=bash,caption={Generic Example of the First GUI Run},label={lst:cap2_primeira_execucao_gui}]
./source/applications/gui/genesys/genesys-gui
\end{lstlisting}
