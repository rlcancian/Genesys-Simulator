...
Once the compilation is complete, the next objective is to start the graphical application. The generated executable corresponds to the \texttt{genesys\_gui\_application} target, and its runtime name is \texttt{genesys-gui}. From the preset build directory, it is available at \path{build/gui-app/source/applications/gui/genesys/genesys-gui}.
