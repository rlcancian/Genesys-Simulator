...
\texttt{ModelSimulation}'s documentation is particularly insightful. She describes it as the mature, functional simulation execution layer, based on repetitions, replication length, \textit{warm-up} configuration, and event processing. It also emphasizes that although there are higher abstractions of experimentation in development, \texttt{ModelSimulation} remains the stable core of execution. This observation is extremely valuable for the developer, because it indicates which operational class actually concentrates the mature behavior of the simulation today.
