...
| 2026-07-21/22 | Map duplicate plugin targets and GLPK divergence | GitHub | issues #487/#492; PRs #488–#491 | through `8520c542a80697aad16527a89cec6b4d675d5797` | CMake File API, symbols, link artifacts | implementation blocked on issue #492 |
...
Append or amend one row only after a material status change.

After merge:

- use the merge commit rather than an unmerged branch head;
- record the strongest relevant validation, not every run;
- identify the principal unresolved boundary;
- update `STATUS.md` and the applicable backlog in the same documentation update when practical.
