...
QColor pluginCategoryColor(const QString& categoryPath) {
    const QString top = canonicalCategoryPath(categoryPath).split('/', Qt::SkipEmptyParts).value(0);
    ...
    if (top == QStringLiteral("DiscreteProcessing")) {
        return QColor(Qt::darkGreen);
    }
...
Model* GraphicalModelSerializer::loadGraphicalModel(const std::string& filename) const {
    ...
    if (model != nullptr) {
        ...
        const bool hasExplicitVisualOverlay = !geometries.isEmpty()
                                               || !animations.isEmpty()
                                               || !counters.isEmpty()
                                               || !variables.isEmpty()
                                               || !timers.isEmpty()
        ...
        for (auto it = persistedComponentsById.constBegin(); it != persistedComponentsById.constEnd(); ++it) {
            ...
            if (_simulator != nullptr && _simulator->getPluginManager() != nullptr && existingComponent->getComponent() != nullptr) {
                Plugin* plugin = _simulator->getPluginManager()->find(existingComponent->getComponent()->getClassname());
                if (plugin != nullptr && !hasExplicitVisualOverlay) {
                    existingComponent->setColor(pluginCategoryColor(QString::fromStdString(plugin->getPluginInfo()->getCategory())));
