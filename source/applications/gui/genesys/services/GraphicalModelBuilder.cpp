...
namespace {
    ...
    bool bottomMostScenePosition(const QList<DataDefinitionLayoutLink>& links, qreal* bottom) {
        if (bottom == nullptr) {
            return false;
        }

        bool found = false;
        qreal localBottom = 0.0;
        for (const DataDefinitionLayoutLink& link : links) {
            if (link.graphicalDefinition == nullptr) {
                continue;
            }
            const qreal candidateBottom = link.graphicalDefinition->sceneBoundingRect().bottom();
            if (!found || candidateBottom > localBottom) {
                localBottom = candidateBottom;
                found = true;
            }
        }
    ...
    void pushLinksBelowSceneY(const QList<DataDefinitionLayoutLink>& links, qreal minimumTopY) {
        qreal maxShift = 0.0;
        bool found = false;
        for (const DataDefinitionLayoutLink& link : links) {
            if (link.graphicalDefinition == nullptr) {
            ...
            found = true;
            const qreal currentTop = link.graphicalDefinition->sceneBoundingRect().top();
            maxShift = std::max(maxShift, minimumTopY - currentTop);
        }

        if (!found || maxShift <= 0.0) {
            return;
        }
        ...
        for (const DataDefinitionLayoutLink& link : links) {
            ...
            const QPointF shiftedPosition = link.graphicalDefinition->pos() + QPointF(0.0, maxShift);
            setAutomaticPosition(link.graphicalDefinition, shiftedPosition);
        }
    }
...
void GraphicalModelBuilder::synchronizeGraphicalDataDefinitionsLayer(Simulator* simulator, ModelGraphicsScene* scene) {
    ...
    for (const auto& componentEntry : componentMap) {
        ...
        positionNewDataDefinitionsInArc(componentBounds,
                                        ...
                                        &occupiedLayoutRects,
                                        1);
        qreal lowerStatisticsRowBottom = 0.0;
        if (bottomMostScenePosition(lowerStatisticsLinks, &lowerStatisticsRowBottom)) {
            pushLinksBelowSceneY(lowerSharedLinks, lowerStatisticsRowBottom + automaticLayoutCollisionPadding);
        }
    ...
    for (const auto& dataDefinitionEntry : dataDefinitionMap) {
        ...
        if (parentUsesUpperArc) {
            ...
            positionNewDataDefinitionsInArc(parentBounds,
                                            ...
                                            &occupiedLayoutRects,
                                            1);
            qreal lowerStatisticsChildRowBottom = 0.0;
            if (bottomMostScenePosition(lowerStatisticsChildLinks, &lowerStatisticsChildRowBottom)) {
                pushLinksBelowSceneY(lowerSharedChildLinks, lowerStatisticsChildRowBottom + automaticLayoutCollisionPadding);
            }
        }
