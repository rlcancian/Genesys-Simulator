...
int AnElectronicAssemblyAndTestSystem::main(int argc, char** argv) {
	...
	assign1->connectTo(seize1);
	seize1->connectTo(delay1);
	delay1->connectTo(release1);
	release1->connectTo(seize3);
	seize3->connectTo(delay3);
	delay3->connectTo(release3);
	release3->connectTo(decide1);
	create2->connectTo(assign2);
	assign2->connectTo(seize2);
	seize2->connectTo(delay2);
	delay2->connectTo(release2);
	release2->connectTo(seize3);
	decide1->connectTo(seize4);
	seize4->connectTo(delay4);
	delay4->connectTo(release4);
	release4->connectTo(decide2);
	decide2->connectTo(record1);
	record1->connectTo(dispose1);
	decide2->connectTo(record2);
	record2->connectTo(dispose2);
	decide1->connectTo(record3);
	record3->connectTo(dispose3);
