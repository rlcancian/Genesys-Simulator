...
SessionManager::SessionManager(TokenService& tokenService, SimulatorFactory simulatorFactory)
    ...
    if (ec) {
        throw std::runtime_error("Failed to create workspace root directory for worker sessions");
    }
}
...
/**
 * @brief Returns the base workspace directory used for all worker sessions.
 */
std::filesystem::path SessionManager::_buildWorkspaceRoot() {
    return std::filesystem::temp_directory_path() / "genesys_worker_sessions";
}
