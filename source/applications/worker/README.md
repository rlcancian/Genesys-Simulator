# Genesys Worker Application

First functional implementation of the **Worker** application type for GenESyS.

## Current scope
- CMake target: `genesys_worker_app` (enabled with `-DGENESYS_BUILD_WORKER_APPLICATION=ON`).
- Executable name: `genesys-worker`.
...
The Worker application is now also being evolved as a **worker API** that can be discovered by a future distributed orchestrator application.
...
## How to run
```bash
cmake -S . -B build/worker-debug -G Ninja -DGENESYS_BUILD_WORKER_APPLICATION=ON
cmake --build build/worker-debug --target genesys_worker_app
