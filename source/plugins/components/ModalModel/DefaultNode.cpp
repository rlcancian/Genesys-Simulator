...
void DefaultNodeTransition::setSource(DefaultNode *source) { _source = source; }

DefaultNode *DefaultNodeTransition::getSource() const { return _source; }
...
void DefaultNodeTransition::setName(std::string name) { _name = name; }

std::string DefaultNodeTransition::getName() const { return _name; }
...
DefaultNode::DefaultNode(Model *model, std::string componentTypename,
                         std::string name)
    : ModelComponent(model, componentTypename, name), DEFAULT() {}

DefaultNode::DefaultNode(Model *model, std::string name)
    : ModelComponent(model, Util::TypeOf<DefaultNode>(), name), DEFAULT() {}
...
bool DefaultNode::isInitialNode() const { return _initialNode; }

void DefaultNode::setFinalNode(bool finalNode) { _finalNode = finalNode; }

bool DefaultNode::isFinalNode() const { return _finalNode; }

//
// public: /// virtual methods
//

std::string DefaultNode::show() { return ModelComponent::show() + ""; }
...
PluginInformation *DefaultNode::GetPluginInformation() {
  PluginInformation *info = new PluginInformation(Util::TypeOf<DefaultNode>(),
                                                  &DefaultNode::LoadInstance,
                                                  &DefaultNode::NewInstance);
...
ModelComponent *DefaultNode::LoadInstance(Model *model,
...
void DefaultNode::_saveInstance(PersistenceRecord *fields,
                                bool saveDefaultValues) {
  ModelComponent::_saveInstance(fields, saveDefaultValues);
  fields->saveField("initialNode", _initialNode, DEFAULT.initialNode,
                    saveDefaultValues);
  fields->saveField("finalNode", _finalNode, DEFAULT.finalNode,
                    saveDefaultValues);
}

void DefaultNode::_onDispatchEvent(Entity *entity,
                                   unsigned int inputPortNumber) {
  traceSimulation(
      this, "I'm just a dummy model and I'll just send the entity forward");
  this->_parentModel->sendEntityToComponent(
