...
static bool isGuiTabularHeader(const std::string& line, std::map<std::string, unsigned int>* columnIndexes) {
	...
	const bool hasSemanticColumn = indexes.find("dataset") != indexes.end()
	    || indexes.find("variable") != indexes.end()
	    || indexes.find("replication") != indexes.end()
	    || indexes.find("time") != indexes.end();
...
static bool parseRecordDataset(const std::vector<TextLine>& lines, const std::string& fileName,
	...
	for (const TextLine& line : lines) {
		...
		if (stripped.front() == '#') {
			...
			// ExpressionName is kept as a legacy field so old Record outputs retain their analytical name.
...
bool SimulationResultsParser::loadFromTextFile(
    ...
    std::string* errorMessage)
{
	...
	// Format detection is ordered by contract: semantic CSV first, Record metadata
	// second, raw numeric last. This prevents old numeric heuristics from stealing
	// a file after a richer format has been identified.
	const SimulationResultsDatasetFormat format = detectFormat(lines);

	bool ok = false;
	if (format == SimulationResultsDatasetFormat::GuiTabular) {
		ok = parseGuiTabularDataset(lines, fileName, dataset, errorMessage);
	} else if (format == SimulationResultsDatasetFormat::RecordLegacy) {
		ok = parseRecordDataset(lines, fileName, dataset, errorMessage);
	} else {
		ok = parseRawNumericDataset(lines, fileName, dataset, errorMessage);
	}
