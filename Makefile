...
# Prefer Ninja when available, otherwise fall back to the default Makefiles generator.
GUI_CMAKE_GENERATOR := $(shell if command -v ninja >/dev/null 2>&1; then printf 'Ninja'; else printf 'Unix Makefiles'; fi)
...
$(GUI_CMAKE_CACHE):
	cmake -S . -B $(GUI_BUILD_DIR) -G $(GUI_CMAKE_GENERATOR) \
